...
#                        self.backLogger.info(QDateTime.currentDateTime().toString('MMddyyyy hh:mm:ss')+","+query.lastQuery())
    ...
    def printLabel(self):
        '''
            printLabel prints a label for whole fish or Methot samples
        '''
        labelTypes = ['Whole Fish Sample',  'Methot Sample']

        listDialog = listseldialog.ListSelDialog(labelTypes, 'Short',  self)
        listDialog.label.setText('Choose Label Type')
        if listDialog.exec_():
            if (listDialog.itemList.currentRow() < 0):
                #  no label type selected
                return
            else:
                labelType = listDialog.itemList.currentItem().text()
        else:
            #  no label type selected
            return
        ...
        if (labelType == 'Whole Fish Sample'):

            #  ensure that a species is selcted
            if (self.activeSpcName == None):
                #  no species selected - show error dialog
                self.message.setMessage(self.errorIcons[2], self.errorSounds[2], "Please select the "
                        "species you're taking a sample of.", 'info')
                self.message.exec_()
                return
            else:
        ...
        elif (labelType == 'Methot Sample'):

#            #  determine the number of preserve samples
#            query=QtSql.QSqlQuery("SELECT count(basket_type) FROM baskets  WHERE " +
#                "SHIP="+self.ship+" AND SURVEY="+self.survey+ " AND event_id="+self.activeHaul+" AND "+
#                "basket_type='Preserve'")
#            if query.first():
#                nPreserve = query.value(0).toInt()
#            else:
#                self.message.setMessage(self.errorIcons[2], self.errorSounds[2], "There are no 'Preserve' " +
#                    "samples to print a label for.", 'info')
#                self.message.exec_()
#                return
#            
#            
#            query=QtSql.QSqlQuery("SELECT sample_id, basket_id, weight FROM baskets  WHERE " +
#                "SHIP="+self.ship+" AND SURVEY="+self.survey+ " AND event_id="+self.activeHaul+" AND "+
#                "basket_type='Preserve'")
#            if query.first():
#                EQDateTime = query.value(0).toString()
...
#
#            #  ask how many fish are being frozen
#            self.numpad.msgLabel.setText("Methot sample weight (kg)")
#            if not self.numpad.exec_():
#                return
#            number = self.numpad.value
