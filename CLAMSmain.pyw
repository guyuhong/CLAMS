...
class CLAMSMain(QMainWindow, ui_CLAMSMain.Ui_clamsMain):
    ...
    def processHaul(self):
        ...
        #  check is this station is a event entry station. Only stations with the haul
        #  module can set the active event. Other stations just follow what is set as the
        #  ActiveEvent in the application_configuration table.
        if not 'haul' in self.modules:
            ...
            if (self.activeEvent == '0'):
                self.message.setMessage(self.errorIcons[2], self.errorSounds[2],
                        ...
                        " process from a haul processing station.", 'info')
                self.message.exec_()
                return
        else:

            #  Check if we're enforcing the "Single Event" rule where you can't change the
            #  event number when any CLAMS stations are open. Usally MACE runs with this
            #  parameter set to False.
            query = QtSql.QSqlQuery("SELECT parameter_value FROM " + self.schema +
                    ".application_configuration WHERE parameter=" + "'ForceSingleEvent'")
            query.first()
            forceSingle = query.value(0).toBool()
